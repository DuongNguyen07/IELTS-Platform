...
export default function ContinueLearning({ lesson }: ContinueLearningProps) {
  ...
  const moduleConfig = MODULE_TYPES[moduleType as keyof typeof MODULE_TYPES] || MODULE_TYPES['reading'];

  const renderModuleIcon = (type: string | undefined, props: SvgIconProps = {}) => {
    const config = MODULE_TYPES[(type?.toLowerCase() as keyof typeof MODULE_TYPES)] || MODULE_TYPES['reading'];
    const Icon: SvgIconComponent = config.Icon;
    return <Icon {...props} />;
  };
    ...
    <section className="mb-12">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-gray-900 text-xl font-bold flex items-center gap-2">
          <ICONS.play className="text-blue-600" />
          Continue Learning
        </h3>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row gap-8 items-center hover:shadow-xl transition-shadow shadow-sm">
        ...
        <div className="w-full md:w-64 aspect-video rounded-xl bg-gradient-to-br from-blue-100 to-purple-100 relative overflow-hidden flex items-center justify-center">
          <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-lg">
            <ICONS.play className="text-blue-600 text-4xl" />
          </div>
        </div>
