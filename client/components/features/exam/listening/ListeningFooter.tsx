...
import type { ListeningExam } from '@/shared/data/exams/listening/types';
...
export default function ListeningFooter({
  exam, activeSectionIndex, answers, flagged, onSectionChange, onScrollToQuestion, onSubmit,
}: Props) {
  const [confirmOpen, setConfirmOpen] = useState(false);
  const answeredCount = Object.keys(answers).length;
  const unanswered = exam.totalQuestions - answeredCount;
  const canGoPrev = activeSectionIndex > 0;
  const canGoNext = activeSectionIndex < exam.sections.length - 1;
      ...
      <footer className="h-16 bg-white border-t border-gray-200 px-4 flex items-center justify-between flex-shrink-0 z-10">
        ...
        <div className="flex items-center gap-4">
          {exam.sections.map((section, idx) => {
            ...
            const nums: number[] = [];
            for (let n = section.questionRange.from; n <= section.questionRange.to; n++) nums.push(n);
        ...
        <div className="flex items-center gap-4">
          <div className="flex gap-1">
            <button
              onClick={() => canGoPrev && onSectionChange(activeSectionIndex - 1)}
              disabled={!canGoPrev}
            ...
            <button
              onClick={() => canGoNext && onSectionChange(activeSectionIndex + 1)}
              disabled={!canGoNext}
              className="w-10 h-10 flex items-center justify-center bg-slate-200 hover:bg-slate-300 rounded-full transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            >
              <ArrowForwardIosIcon />
